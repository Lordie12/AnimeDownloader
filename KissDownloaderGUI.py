...
class App(Frame):
    ...
    # create a Combobox with quality to choose from
        self.available_quality = ["1920x1080.mp4", "1280x720.mp4",
                                  "640x360.mp4", "320x180.3pg",
                                  "960x720.mp4", "480x360.mp4",
                                  "320x240.3pg"]
    ...
    def run_download(self):
        ...
        if destination_folder.endswith('/'):
            ...
            destination = destination_folder + "/" + self.title.get() + "/"
        params = [self.user_name.get(),
                  self.user_password.get(),
                  self.title.get(), self.url.get(),
                  str(self.season_num.get()),
                  str(self.episode_min.get()),
                  str(self.episode_max.get()),
                  destination,
                  self.quality_select.get(),
                  self.site_select.get()]
        print(params)
